...
read.in.libraries()
dat = read.csv("alb_weightALL.csv", sep = ",", header = T)
library(dplyr)

set.seed(1)
data=sample_n(dat, 500)
...
x.coord = data$longitude
y.coord = data$latitude
resp = log(data$alb_weight)
...
spatial.fit1 = fit.spatial.process(data=application.data, method=1,
                                   ini.parms=c(100,100,100), covariogram.model="exponential", trend.input="2nd",
...
spatial.fit1.1 = fit.spatial.process(data=application.data, method=1,
                                     ini.parms=c(800,100,100), covariogram.model="gaussian", trend.input="2nd",
                                     phi.upper.limit=5000)
extract.covariogram.parameters(spatial.fit1.1)

# kriging - spherical
spatial.fit1.2 = fit.spatial.process(data=application.data, method=1,
                                     ini.parms=c(100,100,100), covariogram.model="spherical", trend.input="2nd",
                                     phi.upper.limit=5000)
extract.covariogram.parameters(spatial.fit1.2)

# kriging - circular
spatial.fit1.3 = fit.spatial.process(data=application.data, method=1,
                                     ini.parms=c(100,100,100), covariogram.model="circular", trend.input="2nd",
                                     phi.upper.limit=5000)
extract.covariogram.parameters(spatial.fit1.3)

# kriging - matern
spatial.fit1.4 = fit.spatial.process(data=application.data, method=1,
                                     ini.parms=c(1,1,1), covariogram.model="matern", trend.input="2nd",
                                     phi.upper.limit=1000)
extract.covariogram.parameters(spatial.fit1.4)
# spatial.fit1.4$beta.var

spatial.fit1.4$beta
spatial.fit1.4$sigmasq
spatial.fit1.4$nugget
spatial.fit1.4$phi

spatial.fit1.4$parameters.summary
abline(v=20.968, col = "blue")
...
mod = lm(y~ lon + lat) # -8.0002, -0.059, 0.056
summary(mod)
mod = lm(y~ lon + lat + I(lon^2)*I(lat^2)) # -8.0002, -0.059, 0.056
summary(mod)
...
cov.parms = extract.covariogram.parameters(spatial.fit1.4)
...
set.seed(100)
data2=sample_n(dat, 500)
head(data2)
x.pred.vec = data2$longitude
y.pred.vec = data2$latitude
...
### using K-splines
pred.spline = predict.results(fit = spatial.fit1.4, data = application.data, model = "matern", trend.input = "2nd", locations = )
...
for (i in 1:38){
  ...
  int.result = bootstrap.variance(fit=spatial.fit1.4, data=application.data, 
                                  model="matern", method=6, trend.input="2nd", locations=pred.loc2,
